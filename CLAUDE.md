...
The `/users/**` route is rate limited to 10 req/s per IP (burst 20) with counters in Redis, because it is the only route carrying anonymous operations (`register`, `requestPasswordReset`, `resetPassword`) and they drive Keycloak admin API calls. The key is `X-Forwarded-For` when present, falling back to the socket address — behind a proxy every request would otherwise share one quota. **The gateway now needs Redis to start.**
...
The account-lifecycle rules below apply to the authenticated operations; the two reset mutations carry `@PreAuthorize("permitAll()")` explicitly, so that an operation *without* any annotation still reads as a mistake.
